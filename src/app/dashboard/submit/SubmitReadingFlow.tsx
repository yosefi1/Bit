...
import { Camera, Upload, RefreshCw, Send, X } from "lucide-react";
...
interface UploadResponse {
  file: { url: string; originalName: string; contentType: string; size: number };
  ocr: OcrResult;
}
...
export function SubmitReadingFlow({
  ...
  async function runOcr() {
    ...
    try {
      ...
      setUploaded(data as UploadResponse);
      if (data.ocr.reading != null) {
        setReading(String(data.ocr.reading));
        if (data.ocr.reading < previousReading) {
      ...
      <Card>
        ...
        <CardBody className="space-y-4">
          {!preview ? (
            <label
              htmlFor="meter-file"
              className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 px-4 py-10 text-center hover:border-brand-400 hover:bg-brand-50/40"
            >
              <Camera className="h-10 w-10 text-slate-400" />
              <div className="text-sm font-medium text-slate-700">
                {he.submit.uploadHint}
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            {!preview && (
              <Button onClick={() => fileInput.current?.click()}>
                ...
                {he.submit.chooseImage}
              </Button>
            )}
            {preview && !uploaded && (
              <Button onClick={runOcr} disabled={uploading}>
                <RefreshCw className={`h-4 w-4 ${uploading ? "animate-spin" : ""}`} />
                {uploading ? he.submit.reading : he.submit.readMeter}
              </Button>
            )}
            {preview && (
        ...
        <CardBody className="space-y-4">
          {uploaded && (
            <div className="rounded-lg bg-slate-50 p-3 text-xs text-slate-600">
              ...
              {uploaded.ocr.confidence != null && (
                <>
                  {" "}
                  · {he.submit.confidence} {Math.round(uploaded.ocr.confidence * 100)}%
                </>
              )}
            </div>
          )}
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              {he.submit.previousHint}: <strong>{formatKwh(previousReading)}</strong>.
