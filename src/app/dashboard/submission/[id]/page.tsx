    ...
    <div>
      <PageHeader
        ...
        actions={
          ...
          >
            {he.tenant.fullHistory}
          </Link>
        }
      />

      {sp.just && submission.status === "PENDING" && (
        <Alert tone="success" className="mb-4" title={he.tenant.submissionReceived}>
          {he.tenant.submissionPendingDesc}
        </Alert>
      )}
      ...
      <div className="flex flex-col gap-6 lg:grid lg:grid-cols-3">
        ...
        <div className="space-y-4">
          ...
          {submission.status === "REJECTED" && (
            <Alert tone="danger" title={he.status.submission.REJECTED}>
              {submission.rejectionReason ?? he.tenant.rejectedCorrect}
            </Alert>
          )}
        </div>
      </div>
